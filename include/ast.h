...
namespace Ast {
	...
	struct File : public Node {
		...
		virtual ~File() {}
		virtual bool check(Context<std::shared_ptr<Node>>& ctx) {
			if (this->expression)
				return this->expression->check(ctx);
			return true;
		}
		virtual std::shared_ptr<Node> evaluate(Context<std::shared_ptr<Node>>& ctx) {
			if (this->expression)
	...
	std::shared_ptr<Node> recursiveEvaluate(std::shared_ptr<Node> node, Context<std::shared_ptr<Node>>& ctx, bool stopOnFunction = false);
};
